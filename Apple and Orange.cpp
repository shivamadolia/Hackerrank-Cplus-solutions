 ...
 *  1. INTEGER s ->sam's house starting
 *  2. INTEGER t ->sam's house ending
...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int count_a=0, count_o=0,m,n;
    m=apples.size();
    n=oranges.size();
    for(int i=0; i<m; i++){
        if(((t-a)>=apples[i]) && (apples[i]>=(s-a))){
        ++count_a;
        }   
    }
    for(int i=0; i<n; i++){
        if(((b-s)>= -1*oranges[i]) && (-1*oranges[i]>=(b-t))){
        ++count_o;
        }    
    }
